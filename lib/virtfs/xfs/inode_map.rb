  ...
  class InodeMap
    ...
    def valid_inode_size?(sb)
      if (@inode_blkno + @inode_length) > sb.fsb_to_bb(sb.block_count)
        $log.error "VirtFS::XFS::InodeMap - Inode #{@inode_blkno} too large for filesystem" if $log
        raise "VirtFS::XFS::InodeMap - Inode #{@inode_blkno} too large for filesystem"
      end
      true
    end
    ...
    def valid_inode_number?(inode, sb)
      if (@agno >= sb.sb['ag_count']) ||
        ...
        $log.error "VirtFS::XFS::InodeMap - Bad Inode number #{inode} for agno #{@agno} agcount #{sb.sb['ag_count']}, agbno #{@agbno}, agblocks #{sb.sb['ag_blocks']}, agino #{@agino}, agino_to_ino #{sb.agino_to_ino(@agno, @agino)}" if $log
        raise "VirtFS::XFS::InodeMap - Bad Inode number #{inode}"
      end
      true
    end
    ...
    def imap_lookup(agno, agino)
      ...
      raise "VirtFS::XFS::Superblock.imap_lookup: #{error}" if error
      inode_rec = inode_btree_record(cursor)
      if inode_rec.nil?
        raise "VirtFS::XFS::Superblock.imap_lookup: Error #{cursor.error} getting InodeBtreeRecord for inode #{agino}"
      end
      if inode_rec.start_ino > agino ||
         inode_rec.start_ino + @ialloc_inos <= agino
        raise "VirtFS::XFS::Superblock.imap_lookup: InodeBtreeRecord does not contain required inode #{agino}"
      end
      inode_rec.start_ino
    end # imap_lookup
